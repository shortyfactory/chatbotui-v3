...
export const PromptPicker: FC<PromptPickerProps> = ({ }) => {
  ...
  const [promptLibraryDescription, setPromptLibraryDescription] = useState<
    string | null
  >(null)
  ...
  useEffect(() => {
    if (selectedPrompt) {
      callSelectPrompt(selectedPrompt)
    }
  }, [selectedPrompt])

  const [isTyping, setIsTyping] = useState(false)

  const filteredPrompts = [...promptsLibrary, ...prompts].filter(prompt =>
  ...
  const handleOpenChange = (isOpen: boolean) => {
    ...
    if (!isOpen) {
      ...
      setPromptLibraryDescription(null)
    }
  }

  const callSelectPrompt = (prompt: Tables<"prompts" | "prompts_library">) => {
    const regex = /\{\{.*?\}\}/g
    ...
    if (matches) {
      ...
      const promptLibraryItem = promptsLibrary.find(
        ({ id }) => id === prompt.id
      )
      if (promptLibraryItem) {
        setPromptLibraryDescription(promptLibraryItem.description)
      }
        ...
        <div className="bg-background flex flex-col space-y-1 rounded-xl border-2 p-2 text-sm">
          {showPromptVariables ? (
            <Dialog
              ...
              onOpenChange={(open) => {
                setShowPromptVariables(open)
                if (!open) { handleCancelPromptVariables() }
              }}
            >
