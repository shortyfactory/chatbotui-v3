...
import { handleImportChatHistory } from "../../components/chat/chat-helpers"
import {
...
export const ProfileSettings: FC<ProfileSettingsProps> = ({ }) => {
  ...
  const [azureOpenai41OID, setAzureOpenai41OID] = useState(
    profile?.azure_openai_41_o_id || ""
  )
  const [azureOpenai5ID, setAzureOpenai5ID] = useState(
    profile?.azure_openai_5_id || ""
  )
  ...
  const [fileContent, setFileContent] = useState<{ history: { messages: { id: string; content: string; role: string; }[]; model: { id: string; }; id: string; name: string; temperature: number; prompt: string; }[] }>();

  useEffect(() => {
    if (fileContent) {
      const historyItems = fileContent?.history;
      const chats: { name: string; temperature: number; prompt: string; model: string; user_id: string; workspace_id: string; messages: { content: string; role: string; user_id: string; model: string; sequence_number: number; }[] }[] = [];

      historyItems.forEach((item, index) => {
        if (!profile || !selectedWorkspace) return
        const messages: { content: string; role: string; user_id: string; model: string; sequence_number: number; }[] = [];
        ...
        item.messages.forEach((message, msgIndex) => {
          ...
          const sequence_number = msgIndex + 1;
      ...
      const importHistory = async (chats: { name: string; temperature: number; prompt: string; model: string; user_id: string; workspace_id: string; messages: { content: string; role: string; user_id: string; model: string; sequence_number: number; }[] }[]) => {
        await handleImportChatHistory(chats, setChats)
        setIsOpen(false)
      }

      importHistory(chats).catch(console.error)
    }
  }, [fileContent])
  ...
  const handleSave = async () => {
    ...
    const updatedProfile = await updateProfile(profile.id, {
      ...
      azure_openai_41_o_id: azureOpenai41OID,
      azure_openai_5_id: azureOpenai5ID,
              ...
              <div className="ml-8 space-y-3">
                {useAzureOpenai ? (
                  <>
                    ...
                    {
                      <div className="space-y-1">
                        {envKeyMap["azure_gpt_41_o_name"] ? (
                          <Label className="text-xs">
                            Azure GPT-4.1 deployment name set by admin.
                          </Label>
                        ) : (
                          <>
                            <Label>Azure GPT-4.1 Deployment Name</Label>

                            <Input
                              placeholder="Azure GPT-4.1 Deployment Name"
                              value={azureOpenai41OID}
                              onChange={e =>
                                setAzureOpenai41OID(e.target.value)
                              }
                            />
                          </>
                        )}
...

                    {
                      <div className="space-y-1">
                        {envKeyMap["azure_gpt_5_name"] ? (
                          <Label className="text-xs">
                            Azure GPT-5-mini deployment name set by admin.
                          </Label>
                        ) : (
                          <>
                            <Label>Azure GPT-5-mini Deployment Name</Label>

                            <Input
                              placeholder="Azure GPT-5-mini Deployment Name"
                              value={azureOpenai5ID}
                              onChange={e =>
                                setAzureOpenai5ID(e.target.value)
                              }
                            />
                          </>
                        )}
          ...
          <div className="flex items-center space-x-1">
            <ThemeSwitcher />
             ...
             <WithTooltip
              display={
                <div>
                  Download AI Chatbot UI 1.0 data as JSON. Import coming soon!
                </div>
              }
              trigger={
                <IconFileDownload
            ...
            <WithTooltip
              display={
                <div>
                  Import chat history with JSON file
                </div>
              }
              trigger={
                <IconFileUpload
